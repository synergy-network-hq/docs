...
## Universal Meta-Address — Synergy Network
...
**UMA ID Format:** `uma:<chain>:<address>`
...
Each UMA maps to a public key and supports forward and reverse resolution, association with SNS names, and post-quantum cryptographic proof of control.
...
UMA identities used for: token migration, staking delegation, bridge validations, and DAO role verification.

UMA contracts deployed on:
- Ethereum (`UMARegistry.sol`)
- Solana (`uma_registry.rs`)
- Synergy Mainnet (`uma_module.rs`)
