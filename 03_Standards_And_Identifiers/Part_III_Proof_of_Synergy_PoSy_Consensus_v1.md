...
## 6.9 Cluster-Local DAG Mempool and Pre-Ordering

> *This section was added to reflect the DAG Validator Cluster Architecture introduced in the Testnet-Beta phase. The normative implementation reference is the Synergy DAG Validator Cluster Integration specification (Parts 1–3). The normative security baseline is the Synergy Network Security Specification v1.3.*

### 6.9.1 Design Decision

PoSy's cooperative validator clustering (Section 6.3) creates a natural inner boundary for high-throughput transaction dissemination: the validator cluster. As of the Testnet-Beta phase, Synergy adopts a **cluster-local, availability-certified DAG sublayer** that uses this cluster boundary as the scope for a DAG-based mempool and pre-ordering plane.

This design does **not** replace PoSy finality with DAG-native finality (as in Bullshark or DAG-Rider). QC certification remains the sole canonical settlement mechanism. The DAG improves the data availability and ordering efficiency of the transaction pipeline beneath the unchanged PoSy QC path. This is the same architectural principle used by Narwhal/Tusk — separating transaction dissemination from transaction ordering — applied within Synergy's cluster structure.

The decision to adopt cluster-local DAG rather than network-wide DAG is formally documented in ADR-001 and ADR-002 of the DAG Integration specification.

### 6.9.2 How the DAG Interacts with PoSy

The cluster-local DAG augments the PoSy block production pipeline as follows:

**Before DAG (pre-Testnet-Beta):** Transactions entered a flat mempool. The block proposer assembled transactions directly from the mempool. Validators voted on the resulting block via dual-quorum QC.

**With DAG (Testnet-Beta+):** Transactions are encrypted into `DagBatch` objects and carried in `DagVertex` nodes of the cluster DAG. Each vertex is gossiped within the cluster, and cluster validators issue `AvailabilityReceipt` attestations confirming they have fetched and verified the transaction data. Once ≥ 2f+1 receipts are collected, a `DagCertificate` is formed. The block proposer then selects from the **certified DAG frontier** using a deterministic `OrderingCut` algorithm, producing a transaction ordering with verifiable data availability. The resulting block — with DAG roots committed in its header — is voted on via the **unchanged dual-quorum QC path**.

The DAG does not alter PoSy finality. QC certification remains the sole settlement mechanism.

### 6.9.3 Canonical DAG Objects

Six canonical object types constitute the DAG sublayer's protocol:

| Object | Role |
|---|---|
| `DagBatch` | Encrypted transaction container; carries ciphertext commitment and blob reference |
| `DagVertex` | DAG graph node; references a DagBatch and parent vertices; signed by the producing validator |
| `AvailabilityReceipt` | Attestation that a validator has fetched and verified a DagVertex's blob |
| `DagCertificate` | Aggregated proof of ≥ 2f+1 availability receipts; gates vertex inclusion in ordering |
| `OrderingCut` | Deterministic mapping from certified DAG frontier to ordered transaction list |
| `ClusterCheckpoint` | Compact cross-cluster DAG summary; the only DAG artifact that crosses cluster boundaries |

Full field-level schema definitions for all six types are provided in the DAG Integration specification, Part 1, Section 5.

### 6.9.4 Relationship to Cooperative Clustering

The cluster boundary (Section 6.3) is the DAG scope boundary. This is not coincidental — the cluster is the natural BFT boundary, and the DAG's 2f+1 availability threshold maps directly to the BFT fault tolerance of the cluster. DAG vertex production is gated on cluster membership and epoch registration, consistent with PoSy's validator lifecycle controls.

Cluster rotation (Section 6.3.3) applies to the DAG as well: epoch boundaries are hard DAG graph boundaries. At each epoch, the DAG is sealed with a `ClusterCheckpoint` and a new epoch begins with an empty DAG and (potentially) new cluster membership. This preserves the cartel-prevention properties of cluster rotation within the DAG layer.

### 6.9.5 MEV Prevention via Encrypted Batches

The DAG's pre-ordering visibility surface is mitigated by the encrypted batch invariant: all transaction data entering the DAG must be encrypted as `DagBatch` objects. Batch decryption keys are released only after the QC finalization of the block containing the `OrderingCut` that included the batch. This prevents front-running and generalized MEV extraction via the DAG. This is a mandatory control derived from the Synergy Network Security Specification v1.3, Category 6 (MEV Leakage).
