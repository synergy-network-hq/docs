...
**20. SynQ Smart Contract Language & Virtual Machine**
...
Deployed SynQ contracts are **immutable by design**. Once a contract is finalized on-chain, its bytecode, logic, and declared invariants cannot be modified, patched, or upgraded except through the deployment of a new contract and an explicit governance- or identity-authorized migration of any associated authority or state references. The original contract remains on-chain and continues to execute deterministically under its original terms.

This immutability guarantee is unconditional within the execution plane. It does not interact with the governance layer. Governance has no authority to modify deployed contract code or reverse the state transitions produced by a contract's execution. The consequences of a contract's execution are final once included in a finalized block.

The "rollback semantics" described above refer exclusively to **transaction-level atomicity**: if a transaction's execution encounters a declared failure condition, the entire transaction reverts without partial side effects, preserving the pre-transaction state. This is a correctness mechanism within a single transaction, not a capability to reverse state across multiple finalized blocks.

This design is intentional. The correctness guarantees that make SynQ formally verifiable depend on the invariant that once verified, deployed contracts behave according to their verified specification for the lifetime of their deployment. A mutable contract cannot provide this guarantee.
...
**20.3 Contract Immutability**

SynQ contracts are immutable once deployed. The bytecode of a deployed contract cannot be modified, upgraded, or replaced after deployment. This is a deliberate and non-negotiable design property, not an implementation constraint.

Contract immutability serves several critical functions in the Synergy architecture:

- it ensures that formal verification performed at compile-time remains valid for the lifetime of the deployment — a contract whose behavior can change post-deployment cannot be formally verified in a meaningful sense,
- it prevents governance from unilaterally altering the behavior of contracts that users have relied upon,
- it preserves the deterministic composability of cross-chain contract interactions, which depend on the stability of contract interfaces and semantics over time,
- it provides strong guarantees to counterparties and auditors that the contract they evaluated is the contract that executes.

**Transaction Atomicity vs. Contract Immutability**

SynQ distinguishes between two related but distinct concepts that must not be conflated.

*Transaction-level atomicity* means that if a contract execution fails — for any reason, including an unmet authorization precondition, a violated invariant, or a declared failure condition — the entire transaction is reverted and no state changes take effect. This is a runtime safety property: failed executions leave no partial side effects.

*Contract immutability* means that the contract's logic cannot be changed after deployment. These are different concepts operating at different levels of the system.

Transaction atomicity does not imply contract mutability. A contract can fail gracefully and revert its transaction without the contract itself being modifiable. SynQ enforces both properties simultaneously: contracts execute atomically and are immutable after deployment.

**Upgradeability Through Versioned Redeployment**

Applications requiring evolving logic must implement upgradeability at the application level through explicitly designed patterns, such as deploying new contract versions and migrating state through governed migration contracts. The protocol provides no implicit upgrade mechanism. Any upgrade pattern is itself subject to the same immutability guarantee — the upgrade logic, once deployed, cannot be altered.
