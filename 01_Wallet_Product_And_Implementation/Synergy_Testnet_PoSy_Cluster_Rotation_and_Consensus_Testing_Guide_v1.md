# Synergy Devnet - PoSy Cluster Rotation & Consensus Testing Guide
...
The devnet starts with **1 bootstrap validator** (bootnode):
...
- **Minimum cluster size**: 1 validator (bootstrap)
...
4. **Test Validator Failure Tolerance**
   ```bash
   # Stop one validator (e.g., a non-bootstrap validator)
   # Quorum should still be met if >= 67% of validators are online
...
**Expected Results:**
- ✅ Blocks produced with full validator participation
- ✅ Blocks still produced when the quorum threshold is met (default 67%)
- ❌ Block production halts when participation drops below quorum
