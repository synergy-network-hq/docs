...
2. **Check System Resources**
   ...
   watch -n 1 'ps aux | grep synergy-devnet'

   # Check for memory leaks
   valgrind --tool=memcheck ./target/release/synergy-devnet start
   ```
...
**Symptoms**: The validator control panel reports `Connection to https://devnet-rpc.synergy-network.io/rpc timed out` when you try to register a validator. The bundled `.env` (see `archive/ENV_FILE_REVIEW.md` under the `SYNERGY_RPC_ENDPOINT` block) points at the upstream RPC proxy, so the control panel immediately tries to speak to that host.

**Diagnosis**: The upstream gateway is currently unresponsive. Run the same RPC request manually and watch it hang:

```bash
curl -s -X POST https://devnet-rpc.synergy-network.io/rpc \
...
1. Start a local RPC/bootnode before launching the control panel so that `/rpc` is available on `localhost:48638`. The repo already provides `scripts/reset-devnet.sh` (runs `archive/start-bootnodes.sh` and prepares `data/chain`) or `./archive/start-bootnodes.sh start`.
...
When the upstream proxy is healthy again, you can switch the variables back to `https://devnet-rpc.synergy-network.io` (and the matching WS endpoint).

4. If the control panel runs on remote operator devices, make sure they resolve `devnet-rpc.synergy-network.io` to this machine and can reach ports `48638`/`58638`. Do not change the control panel to `localhost` from those devices—the RPC endpoint must point to this host over the network (proxy or firewall rules permitting) so that registrations come through the real devnet node you're running locally.
...
2. **GitHub Issues**: [Report bugs](https://github.com/synergy-network/devnet/issues)
   - Use issue templates
...
1. **Node Version**: `synergy-devnet --version`
