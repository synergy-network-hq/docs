# Synergy Devnet Startup Guide

## 📋 Table of Contents

1. [Quick Start](#quick-start)
2. [Building the Devnet](#building-the-devnet)
3. [Starting the Devnet](#starting-the-devnet)
...
- **Ports**: 38638 (P2P), 48638 (RPC), 58638 (WebSocket), 9090 (Metrics)

### Installation

```bash
# Clone the repository (if not already done)
cd ~/Desktop/Synergy/synergy-devnet
...
## ▶️ Starting the Devnet

### Method 1: Using Node Templates (Recommended)

The devnet supports multiple node types via templates:
...
./target/release/synergy-devnet start --config config/node_config.toml
```
...
**Current Devnet Bootnode:**
```
snr://synv5um22g62fwrnq6zh92msp9ek7lqrjrw3hpukd@bootnode1.synergy-network.io:38638
```
...
cat devnet/rpc-gateway/node_identity.toml
```
...
./target/release/synergy-devnet keygen --class 5 --output ./new-bootnode-keys
...
#### Method 2: Generate Complete Devnet Identity Set
...
# Or use the devnet key generation tool
cd ../..
cargo run --bin generate_node_keys
```

This will regenerate all node identities in the `devnet/` directory.
...
1. **Update network configuration:**
   ...
   ```toml
   bootnodes = [
     "snr://YOUR-NEW-ADDRESS@YOUR-IP:38638"
   ]
   ```
...
curl -X POST http://localhost:48638 \
...
watch -n 1 'curl -s -X POST http://localhost:48638 \
  -H "Content-Type: application/json" \
  -d "{\"jsonrpc\":\"2.0\",\"method\":\"synergy_blockNumber\",\"params\":[],\"id\":1}" \
  | jq .result'
```
...
./devnet.sh logs follow | grep -i "block"
...
|-----------|----------|----------|---------|--------------|
| Validator-01 |  38638 | 48638 | 58638 | 9090 |
...
lsof -i :48638  # RPC port
lsof -i : 38638  # P2P port
lsof -i :58638   # WebSocket port
lsof -i :9090   # Metrics port

# Or on Linux:
netstat -tulpn | grep -E '38638|33863|58638|9090'
```
...
curl http://localhost:9090/metrics | grep synergy_
```
...
ls -lh ./target/release/synergy-devnet
...
If you need to completely reset the devnet:
...
# Clean all data
./devnet.sh clean  # This will ask for confirmation
...
# Check block height
curl -s -X POST http://localhost:48638 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"synergy_blockNumber","params":[],"id":1}' | jq .result

# Get validators
curl -s -X POST http://localhost:48638 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"synergy_getValidators","params":[],"id":1}' | jq

# Create wallet
curl -s -X POST http://localhost:48638 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"synergy_createWallet","params":[],"id":1}' | jq
...
- GitHub Issues: [synergy-network/devnet](https://github.com/synergy-network/devnet/issues)
